...
class TwoThousandHandler(Handler):
	def withdraw(self, account: BankAccount) -> BankAccount:
		requested_amount = account.get_remaining_amount_to_issue_notes()

		number_of_notes_to_be_issued = int(requested_amount / 2000)
		if number_of_notes_to_be_issued > 0:
			if number_of_notes_to_be_issued > 1:
				print(number_of_notes_to_be_issued,"TwoThousand notes are Issued by TwoThousandHandler")
			else:
				print(number_of_notes_to_be_issued,"TwoThousand note is Issued by TwoThousandHandler")

		pending_amount_to_be_processed = requested_amount % 2000
